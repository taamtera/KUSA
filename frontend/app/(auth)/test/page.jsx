...
export default function test() {
    ...
    const Hours = ["8:00", "8:30", "9:00", "9:30", "10:00", "10:30", "11:00", "11:30", "12:00", "12:30", "13:00", "13:30", "14:00", "14:30", "15:00", "15:30", "16:00", "16:30", "17:00", "17:30", "18:00", "18:30", "19:00", "19:30", "20:00"];
                ...
                <div className="flex justify-center mt-2">
                    <div class="not-prose overflow-auto rounded-lg bg-white outline outline-white/5 dark:bg-gray-950/50">
                        <div class="over7flow-scroll dark:bg-gray-800">
                            ...
                            <div class={`grid max-h-90 grid-cols-[auto_repeat(${Hours.length},50px)] grid-rows-[auto_repeat(${Days.length},50px)] overflow-scroll`}>
                                ...
                                {Hours.map((day, index) => (
                                    <div key={index} class={`sticky top-0 z-10 min-w-20 col-start-${index + 2} row-start-1 border-b border-gray-800 bg-white bg-clip-padding py-2 text-center text-sm font-medium text-gray-900 dark:border-black/10 dark:bg-gradient-to-b dark:from-gray-600 dark:to-gray-700 dark:text-gray-200`}> 
                                        {day}
                                    </div>
                                ))}
                                {Days.map((hour, h_index) => (
                                    <>
                                        ...
                                        {Days.map((day, d_index) => {
                                            if (d_index == Days.length-1 && h_index == Hours.length-1) {
                                                // <div class="col-start-8 row-start-17"></div>
                                                return (<div class={`col-start-${d_index + 2} row-start-${h_index + 2}`}></div>)
                                            } else if (d_index == Days.length-1) {
                                                // <div class="col-start-8 row-start-2 border-b border-gray-800 dark:border-gray-200/5"></div>
                                                return (<div class={`col-start-${d_index + 2} row-start-${h_index + 2} border-b border-gray-200 dark:border-gray-200/5`}></div>)
                                            } else {
                                                // <div class="col-start-2 row-start-2 border-r border-b border-gray-200 dark:border-gray-200/5"></div>
                                                return (<div class={`col-start-${d_index + 2} row-start-${h_index + 2} border-r border-b border-gray-200 dark:border-gray-200/5`}></div>)
                                            }
                                        })}
                                    </>
                                ))}
