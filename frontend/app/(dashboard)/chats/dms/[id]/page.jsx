...
export default function Chat() {
    ...
    const { threadOpen, threadParent, threadReplies, threadLoading, setThreadReplies, setThreadParent, openThread, closeThread } = useMessageThread()
    ...
    useEffect(() => {
        ...
        socket.on("receive_message", (msg) => {
            const isRelevant = msg.sender?._id === otherUserId || msg.context === otherUserId
            if (isRelevant) {
    ...
    useEffect(() => {
        const fetchMessages = async () => {
            try {
                setLoading(true)
                const response = await fetch(`http://localhost:3001/api/v1/chats/dms/${otherUserId}/messages?page=1&limit=50`, { credentials: "include" })
        ...
        <div className="flex flex-col h-screen bg-gray-100">
            ...
            <div className="bg-white p-2 border-b flex items-center justify-between shrink-0">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <h2 className="font-semibold text-black">{otherUser?.display_name || otherUser?.username || "User"}</h2>
            ...
            {/* Thread Modal */}
            {threadOpen && <MessageThread threadParent={threadParent} threadLoading={threadLoading} threadReplies={threadReplies} closeThread={closeThread}></MessageThread>}

            <SearchChatDialog open={isSearchOpen} onOpenChange={setIsSearchOpen} messages={messages} user={user} otherUser={otherUser} />

            <DMsOptions open={isOptionsOpen} onOpenChange={setIsOptionsOpen} otherUserId={otherUserId} />
        </div>
    )
}
