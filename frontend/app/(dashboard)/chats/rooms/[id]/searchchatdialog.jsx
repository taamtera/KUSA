...
export default function SearchChatDialog({ open, onOpenChange, messages, user, otherUser }) {
  ...
  const results = useMemo(() => {
    ...
    return messages.filter((msg) =>
      msg.content.toLowerCase().includes(executedQuery.toLowerCase())
    )
              ...
              <div className="space-y-2 max-h-60 overflow-y-auto">
                {results.map((msg) => {
                  ...
                  return (
                    <div
                      key={msg._id}
                      className="flex items-center gap-3 rounded-lg p-2 hover:bg-gray-100 cursor-pointer transition"
                    >
                      <Avatar className="h-8 w-8">
                        <AvatarImage src={getAvatarUrl(sender?.icon_file)} />
                        <AvatarFallback>{getAvatarFallback(sender?.username)}</AvatarFallback>
                      </Avatar>
                      <div className="flex flex-col">
                        <span className="font-medium text-sm">
                          {isFromCurrentUser ? "You" : sender?.display_name || sender?.username}
                        </span>
