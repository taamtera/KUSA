...
export default function MessageGroup({ sender, messages, fromCurrentUser, onReply, onOpenThread, onEdit, editingTo, isRooms }) {
    const senderName = fromCurrentUser ? "You" : sender?.display_name || sender?.username || "User"
    ...
    return (
        <div className={`flex ${fromCurrentUser ? "justify-end" : "justify-start"} items-start space-x-2`}>
            ...
            {!fromCurrentUser && (
                <div className="flex items-start space-x-2">
                    <Popover open={profileOpen} onOpenChange={setProfileOpen}>
                        <PopoverTrigger asChild>
                            <Avatar className="w-10 h-10 shrink-0 cursor-pointer" onClick={openProfile}>
                                ...
                                <AvatarFallback>{getAvatarFallback(sender?.username)}</AvatarFallback>
                            </Avatar>
                        </PopoverTrigger>
                        <PopoverContent align="start" className="relative z-50 w-[256px] max-w-[50vw] min-w-[320px] max-h-[80vh] rounded-[8px] outline-none bg-white transparent shadow-[0_0px_15px_-3px] shadow-gray-400">
                            <FriendProfile friendInfo={friendInfo} closeProfile={closeProfile} />
                        </PopoverContent>
                    </Popover>
                    ...
                    <div className="flex flex-col items-start">
                        {/* Sender name on top */}
                        {isRooms && (
                            <span className="text-sm font-semibold text-gray-700 dark:text-gray-300 mb-1 cursor-pointer hover:underline hover:font-[1000]" onClick={openProfile}>
                                {senderName}
                            </span>
                        )}
